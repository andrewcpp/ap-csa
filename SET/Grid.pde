public class Grid {
  ...
  public void removeSet() {
    ...
    // YOU WRITE THIS
    if (cardsInPlay > 12 || (deck.size() == 0 && cardsInPlay <= 12)){
      while (!selectedLocs.isEmpty()) {
        int col = selectedLocs.get(selectedLocs.size()-1).getCol();
        int row = selectedLocs.get(selectedLocs.size()-1).getRow();
        if (col == currentCols-1) {
          board[col][row] = null;
        } else {
          for (int i = ROWS - 1; i >= 0; i--) {
            if (board[currentCols-1][i] != null) {
              board[col][row] = board[currentCols-1][i];
              board[currentCols-1][i] = null;
              break;
            }
          }
        }
        selectedLocs.remove(selectedLocs.size()-1);
      ...
      while (!selectedLocs.isEmpty()) {
        if (deck.size() > 0) board[selectedLocs.get(0).getCol()][selectedLocs.get(0).getRow()] = deck.deal();
  ...
  public void addColumn() {
    ...
    if (deck.size() == 0) {
      ...
      message = 3;
      currentCols++;
      for (int num = 0; num < 3; num++) addCardToBoard(deck.deal());
    } else if (findSet().size() > 0) {
      score -= 5;
      message = 1;
    }
  ...
  public boolean isGameOver() {
    // YOU WRITE THIS
    if (deck.size() == 0 && findSet().isEmpty()) return true;
    else return false;
  }
